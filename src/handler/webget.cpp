...
static auto user_agent_str = "Surge Mac/8010";
...
static int curlGet(const FetchArgument &argument, FetchResult &result)
{
    ...
    // 强制设置自定义 User-Agent
    std::string ua_header = std::string("User-Agent: ") + user_agent_str;
    ...
    *result.status_code = code;
    ...
    writeLog(0, "[byby] status_code: " + std::to_string(*result.status_code), LOG_LEVEL_VERBOSE);
